...
\begin{center}
\LARGE{\bfseries{Finding the best parametrization model for ice cream blatter}}
\end{center}
...
\subsection{Q-Q plot}
```{r , echo=FALSE, results="hide", fig.cap="\\label{fig::QQ}Normal Q-Q plot for the residuals from the lmbest", fig.align="center", fig.pos="H"}

lm_best <- lm(viscosity ~ moisture + protein + ash, data=icecream)
tmp <- qqnorm(residuals(lm_best), pch=20, main="Normal Q-Q plot, residuals from best model")
qqline(residuals(lm_best))
diff <- (tmp$x - tmp$y)
...
The labeled points are those that deveviate the most from the line and represent the extreme cases. [Explain why ...]
\subsection{Observed vs Fitted}
```{r, echo=FALSE, results="hide", fig.cap="\\label{fig::fitted}Fitted vs observed ratio", out.extra='trim={0 0.7cm 0 0.7cm},clip', out.width="100%", fig.align="center", fig.pos="H"}
###observed vs fitted
plot(icecream$viscosity ~ fitted(lm_best), pch=20, xlab="Fitted", ylab="Observed", main="Observed vs Fitted viscosity")
...
require(car)
kable(vif(lm_best), col.names = c("VIF"))

```

We can see that the values are even below 5, indicating no colinearity amoung these three factors. Hence, our model does not contain too much parameter, or at least dependant factors. Therefore, VIF cannot really explain the difference between our model outputs and the observed data from figure 3.
\section{Conclusion}
????
