...
Looking at the correlation coefficients of moisture with protein and ash (r=`r correlations["moisture","protein"]` and r=`r correlations["moisture","ash"]`, respectively), we see that they are both negative, close to eacch other and not so strong. Moreover, viscosity does not  have any strong linear relation with the three others variables. The strongest relation between viscisity and a predictor variable is r$_{viscosity-ash}$=`r correlations["viscosity","ash"]`, which is quite small and could let us assess that the futur model would contain more than one predictor variable.
...
require(car)
viftable <- kable(vif(lm.best), col.names = c("VIF"), format = "latex", booktabs = TRUE, digits = 2)
```
